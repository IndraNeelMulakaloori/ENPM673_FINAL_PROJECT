...
Subscribes to a camera topic, publishes std_msgs/Bool on /can_detected,
publishes Twist to /cmd_vel when a large can is detected,
and republishes annotated Image on /debug/can_image.
"""
import rclpy
...
class BlueCanDetectorNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('big_area_thresh', 2000)
        self.declare_parameter('drive_speed', 0.25)
        self.declare_parameter('drive_duration', 4.0)
        self._load_params()
        ...
        self.get_logger().info('BlueCanDetectorNode started')
